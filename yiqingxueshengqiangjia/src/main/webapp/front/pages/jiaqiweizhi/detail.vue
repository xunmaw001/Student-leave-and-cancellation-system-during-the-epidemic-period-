   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
                        ...
                        <view class="text" :style='{"color":"#06A9FF","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.jiaqiweizhiAddress}}</view>
                    </view>
					<view class="detail-list-item"
						  :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
						<view class="lable"
							...
							是否来自疫区：</view>
						<view class="text" :style='{"color":"#06A9FF","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.jiaqiweizhiYesnoValue}}</view>
					</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('jiaqiweizhi', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.jiaqiweizhiPhoto ? this.detail.jiaqiweizhiPhoto.split(",") : [];
			},
